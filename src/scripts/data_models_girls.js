...
.then(([db, err]) => {
	return [
	db.collection('goodslistGirls_0_1'),
	db.collection('goodslistGirls_2_5'),
	db.collection('goodslistGirls_6_9'),
	db.collection('goodslistGirls_10_15'),
	db]; // gets collections,
})
...
.then(docs2 => [docs1, docs2, collection3, collection4, db]))
.then(([docs1, docs2, collection3, collection4, db]) => collection3.find({}).toArray() //passes toArray on one more of them
.then(docs3 => [docs1, docs2, docs3, collection4, db]))
.then(([docs1, docs2, docs3, collection4, db]) => collection4.find({}).toArray() //passes toArray on last of them
.then(docs4 => [docs1, docs2, docs3, docs4, db]))
.then(([docs1, docs2, docs3, docs4, db]) => {
	db.close()
	return [docs1, docs2, docs3, docs4];
})
.then((docs) => {
	var toWrite = 'module.exports = ' + JSON.stringify(docs);
	fs.writeFile('content_girls.js', toWrite, () => {})
})
.catch ((err) => {
